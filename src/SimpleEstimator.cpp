...
SimpleFranzor::SimpleFranzor(std::shared_ptr<SimpleGraph> &g) {
...
void SimpleFranzor::prepare() {

    // nothing to prepare..

}

cardStat SimpleFranzor::computeStats(std::shared_ptr<SimpleGraph> &g) {
...
std::shared_ptr<SimpleGraph> SimpleFranzor::project(uint32_t projectLabel, bool inverse, std::shared_ptr<SimpleGraph> &in) {
...
std::shared_ptr<SimpleGraph> SimpleFranzor::evaluate_aux(RPQTree *q) {
    ...
    if(q->isLeaf()) {
        ...
        return SimpleFranzor::project(label, inverse, graph);
    }

    if(q->isConcat()) {

        // evaluate the children
        auto leftGraph = SimpleFranzor::evaluate_aux(q->left);
        auto rightGraph = SimpleFranzor::evaluate_aux(q->right);

        // join left with right
        return SimpleFranzor::join(leftGraph, rightGraph);
...
cardStat SimpleFranzor::evaluate(RPQTree *query) {
    auto res = evaluate_aux(query);
    return SimpleFranzor::computeStats(res);
}
